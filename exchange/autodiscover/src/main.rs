...
use rpassword;
use xml::writer::{EmitterConfig, XmlEvent as writer_XmlEvent};
use xml::reader::{EventReader, XmlEvent as reader_XmlEvent};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
  ...
  if status == 200 {
    ...
    println!("EWS endpoint URL: {}", get_url_from_autodiscover_response(res_txt)?);
  } else {
    // Request unsuccessful: print the response's code, and optionally its body.
    println!("Failed to retrieve EWS endpoint, server responded with code {}", status);
    let res_txt = res.text().await?;
    if !res_txt.is_empty() {
...
fn build_request(client: &Client, address: &String, password: Option<String>) -> Result<Request, Box<dyn std::error::Error>> {
  ...
  let autodiscover_url = format!("https://autodiscover.{}/autodiscover/autodiscover.xml", domain);
  let request_body = generate_autodiscover_request_body(&address)?;
  let mut req = client.post(autodiscover_url)
...
fn generate_autodiscover_request_body(email: &String) -> Result<String, Box<dyn std::error::Error>> {
  ...
  let events = vec![
    writer_XmlEvent::from(writer_XmlEvent::start_element("Autodiscover").default_ns("http://schemas.microsoft.com/exchange/autodiscover/outlook/requestschema/2006")),
    writer_XmlEvent::from(writer_XmlEvent::start_element("Request")),
    writer_XmlEvent::from(writer_XmlEvent::start_element("EMailAddress")),
    writer_XmlEvent::from(writer_XmlEvent::characters(email)),
    writer_XmlEvent::from(writer_XmlEvent::end_element()),
    writer_XmlEvent::from(writer_XmlEvent::start_element("AcceptableResponseSchema")),
    writer_XmlEvent::from(writer_XmlEvent::characters("http://schemas.microsoft.com/exchange/autodiscover/outlook/responseschema/2006a")),
    writer_XmlEvent::from(writer_XmlEvent::end_element()),
    writer_XmlEvent::from(writer_XmlEvent::end_element()),
    writer_XmlEvent::from(writer_XmlEvent::end_element()),
  ];
...
fn get_url_from_autodiscover_response(res: String) -> Result<String, Box<dyn std::error::Error>> {
  ...
  let parser = EventReader::new(res_buf.as_slice());
  ...
  for e in parser {
    match e {
      Ok(reader_XmlEvent::StartElement { name, .. }) => {
        let tag_name = name.local_name;
        match tag_name.as_str() {
          "Account" => { in_account = true },
          "Protocol" => { if in_account { in_protocol = true }},
          "ASUrl" => { if in_protocol { in_as_url = true }},
          _ => {},
        }
      },
      Ok(reader_XmlEvent::EndElement { name }) => {
        let tag_name = name.local_name;
        match tag_name.as_str() {
      ...
      Ok(reader_XmlEvent::Characters(text)) => {
        if in_as_url {
